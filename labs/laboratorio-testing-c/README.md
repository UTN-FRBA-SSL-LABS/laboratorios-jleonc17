...
> R: Toma un carrito y coloca la cantidad de productos en 0
...
> R: BUG: falta multiplicar por cantidad

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Multiplicar por cantidad, es decir, por c->items[i].cantidad
...
> R: El incorrecto es <=, el que deberia estar es <
...
> R: Las lineas 29 y 30. Son de la funcion carrito_descuento la cual no tiene test asociado en los tests unitarios
...
> R: Los tests unitarios prueban únicamente funciones singularmente. Los test de integración prueban a las funciones en combinación. Lo detectó un test unitario
...
> R: Debido a que en C es el programador quien debe realizar el trabajo de manejo de memoria
...
> R: Escribir tests ejecuta el código prueba su comportamiento, revisarlo visualmente es propenso al error humano y a pasar por alto casos borde
...
> R: Porque el bug requeria que hubiese mas de un solo producto en el carrito
